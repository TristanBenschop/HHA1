	...
	public String receiveEvent(
		...
		if(event.equals("submitOrder")) {
			try {
				...
				uiModel.addAttribute("restaurants", restaurants);
				diningTable = diningTableService.fetchWarmedUp(Long.valueOf(diningTableId));
				uiModel.addAttribute("diningTable", diningTable);		
				restaurant = restaurantService.fetchWarmedUp(diningTable.getRestaurant().getId());
				uiModel.addAttribute("restaurant", restaurant);
		...
		} else if(event.equals("submitBill")) {
			try {
